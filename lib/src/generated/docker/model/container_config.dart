...
class ContainerConfig {
  /// Returns a new [ContainerConfig] instance.
  ContainerConfig({
    this.hostname,
    this.domainname,
    this.user,
  ...
  /// Returns a new [ContainerConfig] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ContainerConfig? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "ContainerConfig[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "ContainerConfig[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ContainerConfig> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final result = <ContainerConfig>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ContainerConfig.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ContainerConfig> mapFromJson(dynamic json) {
    final map = <String, ContainerConfig>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ContainerConfig.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<ContainerConfig>> mapListFromJson(
    ...
    final map = <String, List<ContainerConfig>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ContainerConfig.listFromJson(
          entry.value,
          growable: growable,
        );
      }
