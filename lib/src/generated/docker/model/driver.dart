...
class Driver {
  ...
  int get hashCode =>
      ...
      (name.hashCode) + (options.hashCode);

  @override
  String toString() => 'Driver[name=$name, options=$options]';
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "Driver[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "Driver[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Driver> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final result = <Driver>[];
  ...
  static Map<String, List<Driver>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = Driver.listFromJson(
          entry.value,
          growable: growable,
        );
      }
